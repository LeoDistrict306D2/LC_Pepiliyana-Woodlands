...
/**
 * The club's figures, centred and widely spaced — closer to an exhibition
 * wall text than a stats band. Notes sit under the labels because on this site
 * the caveat matters as much as the number ("trees still standing", not
 * "trees planted").
 *
...
function Figure({ stat }: { stat: Statistic }) {
  ...
  return (
    <div className="text-center">
      <dd className="font-heading text-4xl text-ink md:text-5xl">
        {stat.prefix}
      ...
      {stat.note ? <p className="mt-2 text-xs text-ink-faint">{stat.note}</p> : null}
    </div>
  );
}
